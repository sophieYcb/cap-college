/*
 ...
 File: database/389_publish_mathematics_5e_lot_18.sql
 Purpose: Publish Mathematics 5e lot 18 after 30 grade-A validations.
 Idempotent: Yes.
*/
...
begin
  ...
  where legacy_id between 5000541 and 5000570;

  if question_count <> 30 then
    raise exception
      'Publication annulée : 30 questions attendues, % trouvées.',
      question_count;
  end if;
  ...
  with current_versions as (
    ...
    where question.legacy_id between 5000541 and 5000570
  ),
  latest_reviews as (
  ...
  if approved_count <> 30 then
    raise exception
      'Publication annulée : seulement % validations A sur 30.',
      approved_count;
  end if;
end;
$block$;
...
set review_status = 'approved'::public.review_status,
    published_at = coalesce(question_version.published_at, statement_timestamp())
