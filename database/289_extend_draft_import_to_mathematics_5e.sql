...
-------------------------------------------------------------------------------
 Version      : 1.0.0
 File         : database/289_extend_draft_import_to_mathematics_5e.sql
 Target       : PostgreSQL / Supabase
 Purpose      : Allow the draft importer to accept Mathematics 5e M5 question lots.
 Idempotent   : Yes
===============================================================================
*/
...
declare
  ...
  expected_question_prefix text;
  selected_legacy_base bigint;
...
begin
  ...
  requested_subject_code := requested_payload ->> 'subject';
  requested_level_code := requested_payload ->> 'level';

  case requested_subject_code || ':' || requested_level_code
    when 'mathematics:6e' then
      expected_question_prefix := 'M6';
      selected_legacy_base := 600000;
    when 'mathematics:5e' then
      expected_question_prefix := 'M5';
      selected_legacy_base := 5000000;
    when 'french:6e' then
      expected_question_prefix := 'F6';
      selected_legacy_base := 1000000;
    else
      raise exception 'Draft import is not configured for subject % and level %',
        requested_subject_code, requested_level_code;
  end case;

  select s.id into selected_subject_id
  from public.subjects s
  where s.code = requested_subject_code and s.active;

  select l.id into selected_level_id
  from public.levels l
  where l.code = requested_level_code and l.active;
  ...
  if exists (
    ...
    where (q.item ->> 'code')
            !~ ('^' || expected_question_prefix || '-[0-9]{4,}$')
    ...
    raise exception
      'Every question needs a valid % code, four choices, one answer and difficulty 1-3',
      expected_question_prefix;
  end if;
  ...
  select
    ...
    m.item ->> 'teacherName',
    m.item ->> 'studentName',
    'Micro-compétence atomique de ' ||
      case
        when requested_subject_code = 'french' then 'français '
        else 'mathématiques '
      end || requested_level_code || '.',
