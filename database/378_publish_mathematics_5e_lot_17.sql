/*
 ...
 File: database/378_publish_mathematics_5e_lot_17.sql
 Purpose: Publish Mathematics 5e lot 17 after 20 grade-A validations.
 Idempotent: Yes.
*/
...
begin
  ...
  where legacy_id between 5000521 and 5000540;

  if question_count <> 20 then
    raise exception
      'Publication annulée : 20 questions attendues, % trouvées.',
      question_count;
  end if;
  ...
  with current_versions as (
    ...
    where question.legacy_id between 5000521 and 5000540
  ),
  latest_reviews as (
  ...
  if approved_count <> 20 then
    raise exception
      'Publication annulée : seulement % validations A sur 20.',
      approved_count;
  end if;
end;
$block$;
