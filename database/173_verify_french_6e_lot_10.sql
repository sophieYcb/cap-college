/*
 ...
 File: database/173_verify_french_6e_lot_10.sql
 Purpose: Verify French 6e lot F6-GRA-10 after import.
 Read-only: Yes
*/

with lot_questions as (
  select q.id, q.status, ms.code as micro_skill_code,
         q.current_version_number
  from public.questions q
  join public.micro_skills ms on ms.id = q.micro_skill_id
  where q.legacy_id between 1000298 and 1000308
),
current_versions as (
