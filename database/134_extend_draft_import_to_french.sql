...
-------------------------------------------------------------------------------
 Version      : 1.0.0
 File         : database/134_extend_draft_import_to_french.sql
 Target       : PostgreSQL / Supabase
 Purpose      : Allow the draft importer to accept French 6e F6 question lots.
 Idempotent   : Yes
===============================================================================
*/
...
begin
  ...
  case requested_subject_code
    ...
    when 'french' then
      ...
      raise exception 'Draft import is not configured for subject %',
  ...
  if requested_payload ->> 'level' <> '6e' then
    raise exception 'This importer currently accepts 6e lots only';
  end if;
  ...
  select
    ...
    'Micro-compétence atomique de ' ||
      case
        when requested_subject_code = 'french' then 'français 6e.'
        else 'mathématiques 6e.'
      end,
