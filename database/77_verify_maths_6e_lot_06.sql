with lot_questions as (
  select id, current_version_number, status, micro_skill_id from public.questions
  where legacy_id between 600201 and 600240
), versions as (
  select v.id,v.question_id from public.question_versions v join lot_questions q on q.id=v.question_id
  where v.version_number=q.current_version_number
), counts as (
  select v.question_id,count(c.id) choices,count(*) filter(where c.is_correct) correct_choices,
    max(c.sort_order) filter(where c.is_correct) correct_position
  from versions v join public.answer_choices c on c.question_version_id=v.id group by v.question_id
)
select jsonb_build_object(
  'questions',(select count(*) from lot_questions),'versions',(select count(*) from versions),
