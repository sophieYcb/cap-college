/*
 ...
 File: database/314_publish_mathematics_5e_lot_06.sql
 Purpose: Publish Mathematics 5e lot M5-REL-03 after 10 grade-A validations.
 Idempotent: Yes.
*/
...
begin
  ...
  where legacy_id between 5000191 and 5000200;

  if question_count <> 10 then
    raise exception
      'Publication annulée : 10 questions attendues, % trouvées.',
      question_count;
  end if;
  ...
  with current_versions as (
    ...
    where question.legacy_id between 5000191 and 5000200
  ),
  latest_reviews as (
  ...
  if approved_count <> 10 then
    raise exception
      'Publication annulée : seulement % validations A sur 10.',
      approved_count;
  end if;
end;
$block$;
