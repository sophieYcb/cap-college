/*
 ...
 File: database/171_publish_french_6e_lot_09.sql
 Purpose: Publish French 6e lot F6-GRA-09 after 27 grade-A validations.
 Idempotent: Yes
*/
...
begin
  ...
  where legacy_id between 1000271 and 1000297;

  if question_count <> 27 then
    raise exception
      'Publication annulée : 27 questions attendues, % trouvées.',
      question_count;
  end if;
  ...
  latest_reviews as (
    select distinct on (qr.question_version_id)
      qr.question_version_id, qr.grade, qr.status
  ...
  if approved_count <> 27 then
    raise exception
      'Publication annulée : seulement % validations A sur 27.',
      approved_count;
  end if;
end;
$block$;
