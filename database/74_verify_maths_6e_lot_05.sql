/*
===============================================================================
 CAP-COLLEGE DATABASE - VERIFY MATHS 6E LOT 05
-------------------------------------------------------------------------------
 Version      : 1.0.0
 File         : database/74_verify_maths_6e_lot_05.sql
 Purpose      : Verify the fifth mathematics 6e lot after JSON import.
 Read only    : Yes
===============================================================================
*/
...
with lot_questions as (
  ...
  where legacy_id between 600161 and 600200
),
lot_versions as (
...
choice_counts as (
  select version.question_id, count(choice.id) choices,
    count(*) filter (where choice.is_correct) correct_choices,
    max(choice.sort_order) filter (where choice.is_correct) correct_position
...
position_counts as (
  select
    count(*) filter (where correct_position = 1) answer_a,
    count(*) filter (where correct_position = 2) answer_b,
    count(*) filter (where correct_position = 3) answer_c,
    count(*) filter (where correct_position = 4) answer_d
  from choice_counts
)
